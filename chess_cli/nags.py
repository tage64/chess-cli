...
nag_descriptions: dict[int, str] = {
    ...
    7: "forced move (Gall others lose quickly) or only move",
...
def ascii_glyph(nag: int) -> str:
    if nag in ascii_nags:
        return ascii_nags[nag]
    return "$" + str(nag)


def parse_nag(text: str) -> int:
    if text in nag_asciis:
        return nag_asciis[text]
    if not text.startswith("$"):
        raise ValueError("Invalid NAG. Must either be an ascii glyph or start with a '$'")
    return int(text[1:])
